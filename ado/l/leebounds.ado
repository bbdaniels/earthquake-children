...
if !replay() {
		quietly {
			...
			** TEMPORARY NAMES **
			tempvar  __tempsel __tempt __selnum __cesamp __esamp __zz __wwsc __wwusc __tempid __intwc
			...
			** CHECK and GENERATE SELECTION INDICATOR **
			if "`select'" != "" {
                egen `__selnum' = group(`select') `in' `if' 
			    replace `__selnum' = `__selnum'-1
				if "`weight'" == "" {
					sum `__selnum' `if2' `yy'<. 
				}
				else {
					sum `__selnum' `if2' `yy'<. & `exp' >=0 & `exp' <. [iw=`exp']				
				}
				local cssel = r(sum)
				tab `__selnum' `if' `in', matrow(`__selcat')
				if r(r) == 1 {
				...
				gen `__tempsel' = `__selnum' `in' `if2' (`__selnum' == 1 | `__selnum' == 0)
				local ss "`__tempsel'"
			}
			else {
            ...
            ** GENERATE DUMMY WEIGHTS & CHECK FOR INTEGER FWEIGHTS **
			if "`exp'" == "" {
				local exp "1"
				local dw "pw"
			}
			else {
                gen `__intwc' = `exp'-int(`exp')
                sum `__intwc'
                if "`weight'" == "fweight" & (r(min) != 0 | r(max) != 0) {
...
version 11.1
	...
	** TEMPORARY NAMES **
	...
	if "`wexp'" == "1" {
        ...
        replace `__ww2' = round(`ww'/r(min)) if `ww' <. & `__ww2' >=.
		keep `id' `__ww2' `gg'
